...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send_file(self, file_path: str):
        # Normalize and ensure backslashes for Windows if needed, 
        # but Python usually handles forward slashes fine. 
        # Let's be explicit for logging.
        ...
        # Security check: only allow serving files from OUTPUT_ROOT
        abs_file = os.path.abspath(file_path).lower()
        abs_root = os.path.abspath(OUTPUT_ROOT).lower()
        
        # Ensure abs_root ends with a separator for safe startswith check
        if not abs_root.endswith(os.path.sep):
            abs_root += os.path.sep
    ...
    def do_GET(self):
        try:
            ...
            # Serve files from OUTPUT_ROOT
            if parsed.path.startswith("/output/"):
                # The path is always expected to be relative to OUTPUT_ROOT
